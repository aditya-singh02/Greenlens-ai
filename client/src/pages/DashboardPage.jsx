...
const MOCK_WARDS = Array.from({ length: 20 }, (_, i) => ({
  wardId: i + 1,
  wardName: ['Sirpur', 'Tejaji Nagar', 'Sudama Nagar', 'Rajendra Nagar', 'Nehru Nagar', 'Gandhi Nagar', 'Vijay Nagar', 'Palasia', 'Bhanwarkuan', 'Mhow Naka', 'Annapurna', 'Bhawarkua', 'Lasudia', 'Kanadiya', 'Banganga', 'Rau', 'Manglia', 'Nipania', 'Scheme 54', 'Juni Indore'][i],
  stressLevel: ['Critical', 'High', 'Moderate', 'Low'][Math.floor(Math.random() * 4)],
  stressScore: 20 + Math.random() * 75,
  densityClass: ['High', 'High-Medium', 'Low-Medium', 'Low'][Math.floor(Math.random() * 4)],
  population2029: 15000 + Math.floor(Math.random() * 85000),
  perCapitaOpenSpace_m2: (1 + Math.random() * 18).toFixed(1) * 1,
  waterGap2029: (0.5 + Math.random() * 12).toFixed(1) * 1,
  greenCover: ['Critical', 'Moderate', 'Good', 'Excellent'][Math.floor(Math.random() * 4)],
}))
      ...
      <div className="bg-white border-b border-green-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              <p className="text-xs font-semibold text-forest-500 uppercase tracking-widest mb-1">Indore City</p>
              <h1 className="font-display text-3xl font-bold text-forest-900">Ward Dashboard</h1>
              <p className="text-forest-500 mt-1 text-sm">
                {loading ? 'Loading...' : `${wards.length} wards · Sorted by stress level`}
              </p>
            </div>
            {error && (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {!loading && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <StatCard icon={AlertTriangle} label="Critical Wards" value={summary.counts?.Critical || 0}
              sub="Immediate action needed" color="red" accent="bg-red-300" />
            <StatCard icon={TreePine} label="Avg Green Space" value={(summary.avgGreen || 0).toFixed(1)}
              unit="m²/person" sub={`WHO min: 9 m²`} color="forest" accent="bg-forest-300" />
            <StatCard icon={Users} label="Pop Covered 2029"
              value={summary.totalPop ? (summary.totalPop / 1000000).toFixed(1) + 'M' : '—'}
              sub="Projected population" color="forest" accent="bg-forest-200" />
            <StatCard icon={Droplets} label="High-Risk Wards"
              value={(summary.counts?.Critical || 0) + (summary.counts?.High || 0)}
              sub="Critical + High stress" color="amber" accent="bg-amber-300" />
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-8">
          ...
          <div className="glass-card p-5 col-span-1">
            <h3 className="font-display font-semibold text-forest-900 text-sm mb-4">Stress Distribution</h3>
            <ResponsiveContainer width="100%" height={130}>
              <BarChart data={chartData} barSize={28}>
                <XAxis dataKey="level" tick={{ fontSize: 11, fill: '#4d7a4d' }} axisLine={false} tickLine={false} />
                <YAxis hide />
                <Tooltip
          ...
          <div className="lg:col-span-2 flex flex-col gap-3 justify-center">
            <div className="relative">
              ...
              />
              {search && (
                <button onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-forest-300 hover:text-forest-600">
                  <X size={15} />
                </button>
              )}
            </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm text-forest-500">
            {loading ? 'Loading wards…' : `Showing ${filtered.length} of ${wards.length} wards`}
            {hasFilters && <span className="ml-2 text-forest-400">· filtered</span>}
          </p>
          <div className="flex items-center gap-3">
            {hasFilters && (
              <button
                onClick={() => { setSearch(''); setStressFilter('All'); setDensityFilter('All') }}
            ...
            <div className="flex items-center gap-1 bg-white border border-green-200 rounded-full p-1">
              <button
                ...
                className={`flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full transition-all ${
                  view === 'grid' ? 'bg-forest-600 text-white' : 'text-forest-500 hover:text-forest-700'
                }`}
              >
              ...
              <button
                onClick={() => setView('map')}
                className={`flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full transition-all ${
                  view === 'map' ? 'bg-forest-600 text-white' : 'text-forest-500 hover:text-forest-700'
                }`}
              >
                <MapIcon size={13} /> Map
              </button>
          ...
          <div className="glass-card p-5 mb-6">
            <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
              <h3 className="font-display font-semibold text-forest-900 text-sm">
                Indore Ward Map · {hasFilters ? `${filtered.length} matching wards highlighted` : 'All 85 wards'}
              </h3>
              <MapLegend />
            </div>
            ...
            <p className="text-xs text-forest-400 mt-3">
              Click any ward on the map to open its detail page. Hover for a quick stress summary.
            </p>
          </div>
        )}
        ...
        {view === 'grid' && (loading ? (
            ...
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i} className="glass-card p-5 animate-pulse">
                ...
                <div className="grid grid-cols-3 gap-2 mb-4">
                  {[1, 2, 3].map(j => <div key={j} className="h-10 bg-green-100 rounded" />)}
                </div>
          ...
          <div className="text-center py-24">
            ...
            <p className="text-forest-400 font-medium">No wards found</p>
            <p className="text-sm text-forest-300 mt-1">Try adjusting your search or filters</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filtered.map(ward => (
